...
function hesaplaKomisyon(fiyat) {
  let komisyonOrani = props.comm
  let komisyonTutari = fiyat * (komisyonOrani / 100);
  let netFiyat = fiyat - komisyonTutari;
  return Math.ceil(netFiyat);
}
  ...
  const [minPrice, setMinPrice] = React.useState(formatCurrencyToNumber(data?.fiyat).toFixed() || 0);
  ...
  const handleClick = async () => {
    ...
    try {
    let darta = JSON.stringify({minPrice, checkedBot, ilanID: data.id, checkedTers})
    let config = {
      method: 'post',
      maxBodyLength: Infinity,
      url: `${process.env.REACT_APP_BACKEND_URI}/dQl7AROodQ`,
      headers: { 
           ...
           <AspectRatio flex ratio="1" maxHeight={182} sx={{ minWidth: 182 }}>
            ...
            <CardContent>
              ...
              <Typography level="body-sm" fontWeight="lg" textColor="text.tertiary">
                Beden: <span style={{ color: '#ffffff' }}>{data?.beden}</span> <br /> Satış Sırası: <span style={{ color: '#ffffff' }}>{data?.sıra}.</span>
              </Typography>
              <Sheet
                sx={{
              ...
              >
                <div>
                  ...
                  <Typography fontWeight="lg">{data?.fiyat}</Typography>
                </div>
                <div>
                  <Typography level="body-xs" fontWeight="lg">
                    Satıcı Payı
                  </Typography>
                  <Typography fontWeight="lg">{formatCurrencyToNumber(data?.net_kazancınız).toFixed(2)} </Typography>
                </div>
                <div>
                  <Typography level="body-xs" fontWeight="lg">
                  %{props.comm} Kom.
                  </Typography>
                  <Typography fontWeight="lg">{(formatCurrencyToNumber(data?.fiyat) - formatCurrencyToNumber(data?.net_kazancınız)).toFixed(2)}</Typography>
                </div>
              ...
              >
                 <div>
                  ...
                  <Typography fontWeight="lg">{data?.en_düşük_fiyat}</Typography>
                </div>
              ...
              >
                  ...
                  <Typography level="xm" fontWeight="lg">
                   ...
                   {hesaplaKomisyon(Number(minPrice))} Min. Fiyatta Satıcı payı
                   
                  </Typography>
                  
                  <Slider
                     aria-label="Always visible"
                     defaultValue={savedProduct ? Number(savedProduct.minPrice) : Number(formatCurrencyToNumber(data?.fiyat).toFixed() || 0)}
                     onChange={(event, newValue) => valueText(newValue)}
                     step={10}
                     max={Number(formatCurrencyToNumber(data?.fiyat).toFixed() || 0)}
