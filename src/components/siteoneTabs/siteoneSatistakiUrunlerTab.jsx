...
const sitetwoSatistakiUrunlerTab = () => {
  ...
  React.useEffect(() => {
    ...
    async function getInfo() {
      try {
        let response = await axios.get(`${process.env.REACT_APP_BACKEND_URI}/OkQo66kc3F`, {
          headers: { 'Authorization': `${savedData ? savedData : ""}` }
        });
        let response2 = await axios.get(`${process.env.REACT_APP_BACKEND_URI}/njiAhSvPyT`, {
          headers: { 'Authorization': `${savedData ? savedData : ""}` }
        });
        if (response2.data) {
          setComm(response2.data.data.comiss)
         }
        if (response.data) {
          let dosta = response.data
          let aka = response.data.data.map(x => {
            let temizlenmisDeger = x.net_kazancınız.replace('₺', '').replace(',', '.');
      ...
      <Box  sx={{ display: 'flex', flexDirection: 'column' }}>
        <CardContent sx={{ flex: '1 0 auto' }}>
          <Typography component="div" variant="h5">
           Toplam {data?.data?.length} Üründe {data?.toplam.toFixed(2)}₺ Kazanç.
          </Typography>
            ...
            {data?.data?.map((product) => (
              ...
              <Urun key={product.id} data={product} savedProduct={data?.savedProducts?.find(x => x.ilanID == product.id)} comm={comm} />
            ))}
