  ...
  React.useEffect(() => {
    ...
    async function getInfo() {
      try {
        ...
        if (response.data) {
          setUserData(response.data.status ? response.data.data.me : null);
          setLoading(false);
        }
      } catch (error) {
          ...
          <StyledPaper elevation={4}>
            <Grid container spacing={2} justifyContent="center">
              ...
              <Grid item xs={12} display="flex" justifyContent="center">
                ...
                <span style={{ fontWeight: 'bold' }}> {userData?.name} {userData?.surname} </span>
                </Typography>
              </Grid>
              <Grid item xs={12} sm={6}  >
                <Typography variant="body1">Email: <span style={{ fontWeight: 'bold' }}>{userData?.email}</span></Typography>
                <Typography variant="body1">Telefon: <span style={{ fontWeight: 'bold' }}>{userData?.phone}</span></Typography>
              </Grid>
              <Grid item xs={12} sm={6}  >
                <Typography variant="body1">Şehir: <span style={{ fontWeight: 'bold' }}>{userData?.city?.name}</span></Typography>
                <Typography variant="body1">İlçe: <span style={{ fontWeight: 'bold' }}>{userData?.district?.name}</span></Typography>
              </Grid>
              <Grid item xs={12} sm={6}  >
                <Typography variant="body1">İban İsim: <span style={{ fontWeight: 'bold' }}>{userData?.ibanData?.ibanName}</span></Typography>
                <Typography variant="body1">İban: <span style={{ fontWeight: 'bold' }}>{userData?.ibanData?.iban}</span></Typography>
              </Grid>
              <Grid item xs={12} sm={6}  >
                <Typography variant="body1">Seviye: <span style={{ fontWeight: 'bold' }}>{userData?.level?.title}</span></Typography>
                <Typography variant="body1">Toplam Komisyon: <span style={{ fontWeight: 'bold' }}>{userData?.level?.commissionRate}</span></Typography>
                <Typography variant="body1">Hizmet Bedeli: <span style={{ fontWeight: 'bold' }}>{userData?.level?.serviceFee} TL</span></Typography>
              </Grid>
